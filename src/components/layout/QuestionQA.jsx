    ...
    <div className="w-[75%] mx-auto py-4 mt-6">
      ...
      <div className="grid grid-cols-12 mt-6 gap-6">
        {QA_DATA.map((item, index) => (
          <div
            className="col-span-12 md:col-span-6 lg:col-span-4 bg-[#F9F6ED] px-8 py-10"
            key={index}
          >
            ...
            <p className="text-[#0D0D0D] text-xl mt-6 md:h-22 lg:h-[82px] line-clamp-3 font-svn-avo font-bold">
              {item.question}
            </p>
            <p className="text-[#6A6A6A] text-xl mt-2 lg:mt-4 font-svn-avo italic">{item.answer}</p>
          </div>
        ))}
      </div>
    </div>
